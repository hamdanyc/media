...
load("media.RData")     # lists of KIC::espn
options(digits=3)
...
def.txt <- news.today %>%
  ...
  filter(sapply(news.today$article, function(x) any(sapply(def.key, stringr::str_detect, string = x)))) %>% 
  select(text)
...
dstr.txt <- news.today %>%
  mutate(text = paste(src, "**", headlines, "**",
                      strtrim(article, 650),"... lagi >>", newslink, sep=":")) %>%
  filter(str_detect(text, paste(dstr.key, collapse = '|' ))) %>%  
  select(text) 
...
|Kategori Berita|Jumlah|
|:--------------|:----:|
|1. **Keselamatan: Dalam Negeri dan Serantau**|`r nrow(def.txt)`|
|    a. **Jenayah**|`r nrow(crime.txt)`|
|    b. **Rentas Sempadan**|`r nrow(imigr.txt)`|
|    c. **Bencana**|`r nrow(dstr.txt)`|
|2. **Ketenteraan**|`r nrow(atm.txt)`|
|3. **Politik**||
|    a. **Hal-ehwal Parti**|`r nrow(pol.txt)`|
|    b. **Pilihanraya**|`r nrow(pru.txt)`|
